    ...
    try {
        ...
        while (-not $process.HasExited) {
            if ([datetime]::UtcNow -gt $deadline) {
                ...
                if ($AllowFailure) {
                    Write-Host "WARNING: $Label skipped after timeout."
                    return $false
                }
...
Invoke-TimedCommand -Label "Verifying whisperx import" `
    -FilePath $VenvPython `
    -ArgumentList @('-c', "import whisperx; print('  whisperx OK:', getattr(whisperx, '__version__', 'installed'))") `
    -TimeoutSec 60
Invoke-TimedCommand -Label "Verifying torch import" `
    -FilePath $VenvPython `
    -ArgumentList @('-c', "import torch; print('  torch OK:', torch.__version__)") `
    -TimeoutSec 60
